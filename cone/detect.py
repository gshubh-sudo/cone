...
def detect(
    ...
    detection=open("output"+"/detections", 'w')
    for i, (path, img, im0) in enumerate(dataloader):
        ...
        if len(pred) > 0:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cl in detections:
                if save_txt:  # Write to file
                    with open(save_path + '.txt', 'a') as file:
                        file.write('%g %g %g %g %g %g\n' %
                                   (x1, y1, x2, y2, cl, cls_conf * conf))

                # Add bbox to the image
                label = plot_one_box([x1, y1, x2, y2],cl, im0)
